...
abstract class StockYardSeeder extends Seeder
{
     ...
     */
    protected function runCSVSeeder(string $filePath): void
    {
    ...
    protected function createDataArray(array $row): array
    {
        if (count($row) !== count($this->capsule)) {
            throw new Exception(
                "CSV column count mismatch row:". count($row) .", csv: ". count($this->capsule)
            );
        }
